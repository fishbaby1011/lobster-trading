...
LOG_DIR="runs/v5_4_launcher_logs"
...
pattern = (
    f"v5_4_repaired_h20_seed{seed}_*"
)
...
run_worker() {
  ...
  for ((
    ...
    name="v5_4_repaired_h20_seed${seed}"
...
summary.to_csv(
    "results/v5_4_repaired_seed_summary.csv",
    index=False,
)

folds.to_csv(
    "results/v5_4_repaired_folds.csv",
    index=False,
)
...
yearly.to_csv(
    "results/v5_4_repaired_yearly.csv",
    index=False,
)
...
out.append(
    "LOBSTER V5.4 REPAIRED-DATA VALIDATION"
)
...
Path(
    "results/v5_4_repaired_compare.txt"
).write_text(
    text + "\n"
)
