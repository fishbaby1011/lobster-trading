...
ALL_FEATURES = list(meta["features"])
...
FEATURES = [
    ...
    if f not in FEATURE_DROPS[args.feature_set]
]

if len(FEATURES) < 4:
    raise RuntimeError(
        f"too few features for {args.feature_set}: {FEATURES}"
    )
...
name = (
    ...
    or (
        f"v5_1_h{H}_"
        f"fs{args.feature_set}_"
        f"tw{args.train_window}_"
        f"seed{SEED}"
    )
)
...
for year in range(
    ...
    if args.train_window != "expanding":
        ...
        train = train[
            train.index >= train_start
        ].copy()

        refit = refit[
            refit.index >= refit_start
        ].copy()
    ...
    out["seed"] = SEED
    out["horizon"] = H
    out["feature_set"] = args.feature_set
    out["train_window"] = args.train_window
